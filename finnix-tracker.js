...
const server = new (require('bittorrent-tracker').Server)({
  ...
  filter: function (infoHash, params, cb) {
    if (allowedHashes.includes(infoHash)) {
      cb(null)
    } else {
...
server.on('update', function (addr, params) {
  console.log('finnix-tracker ' + params.info_hash + ' update: ' + addr)
})
server.on('complete', function (addr, params) {
  console.log('finnix-tracker ' + params.info_hash + ' complete: ' + addr)
})
server.on('start', function (addr, params) {
  console.log('finnix-tracker ' + params.info_hash + ' start: ' + addr)
})
server.on('stop', function (addr, params) {
  console.log('finnix-tracker ' + params.info_hash + ' stop: ' + addr)
})
